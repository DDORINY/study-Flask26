...
#----------------------------------------------------------------------------------------------------------------------
# 성적 CRUD
# 주의 사항 :  role에서 admin과 manager만 CUD를 제공한다.
    ...
    try:
        with conn.cursor() as cursor:
            ...
            if student:
                ...
                row = cursor.fetchone()
                print(row) #테스트용 ㅌ코드로 dict 타입으로 콘솔 출력
                if row :
...
def score_list():
    ...
    try:
        with conn.cursor() as cursor:
            ...
            # 성적이 없는 학생은 제외하고, 성적이 있는 학생만 총점 순으로 정렬
            sql = """
            ...
            print(f"sql결과 테스트 : {datas}")

            #3. DB에서 가져온 딕셔너리 리스트를 Score 객체 리스트로 변환
            score_objects = []
            for data in datas:
                # Score 클래스에 정의하신 from_db 활용
                s=Score.from_db(data) #직렬화 (DICT 타입을 객체로 만들었다)
                # 객체에 없는 정보를 조인으로 가져옴
                s.name =data['name']
                s.uid =data['uid']
                score_objects.append(s) #객체를 리스트에 넣었다.
...
#----------------------------------------------------------------------------------------------------------------------
# 파일 게시판 CRUD
# [v]01. 파일의 업로드와 다운로드 기능!!
# [v]02. 단일파일과 다중파일 업로드 처리!!
# [v]03. 서비스 패키지 활용
# [v]04. /upload라는 폴더를 사용하겠다. / 용량 제한 16MB
# [v]05. 파일명 중복 방지용 코드 활용!
# [v]06. DB에서 부모객체가 삭제가되면 자식객체도 삭제되게 cascade처리 함!
