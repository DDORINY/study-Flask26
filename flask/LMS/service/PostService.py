...
class PostService:
    ...
    def save_post(member_id, title, content, files=None, upload_folder='uploads/'):
        ...
        try:
            ...
            conn.close()  # save_post(member_id,title,content,files,upload_folder') = >True Or False
    ...
    def get_post_detail(post_id):
        ...
        try:
            with conn.cursor() as cursor:
                ...
                conn.commit()
                return post, files # post는 posts 에 있는 자료/file은 첨부파일에 대한 자료
        finally:
            conn.close()
    ...
    def delete_post(post_id, upload_folder='uploads/'):
        ...
        try:
            with conn.cursor() as cursor:
                ...
                # 삭제 시 CASCADE 옵션을 하지 않으면 자식테이블 데이터를 선 삭제 후 부모테이블의 데이터를 삭제한다.
    ...
    def update_post(post_id, title, content, files=None, upload_folder='uploads/'):
        ...
        try:
            with conn.cursor() as cursor:
                ...
                if files and any(f.filename != '' for f in files): #파일명이 바꼈을 때
                    ...
                    for old in old_files:
                        ...
                        if os.path.exists(old_path):
                            os.remove(old_path) # 파일을 하나씩 가져와 삭제함
